...
if __name__=="__main__":
    ...
    print("Chromosome: ",chromosome," up: ", up, " ignored: ", ignored, 'best_fitness: ',best_fitness,'Total games: ',total_games,
    ...
    path=0
    for i in range(1,100): # Range set to 1,100
        total_games+=1
        ...
        insert_chrom = random.choice([x for x in range(0,11) if x not in chromosome])

        #If chromosome is not subset of best chromosme replace chromosome
        ...
        if len(chromosomes_with_score_lower_than_10)>1 and chromosome in chromosomes_with_score_lower_than_10:
            chromosome.clear()
            if len(chromosomes_with_score_higher_than_13)>2:
                chromosome = chromosomes_with_score_higher_than_13[generate_random_number(0,len(chromosomes_with_score_higher_than_13)-1)]
        ...
        #3 Choose a New Chromosome 
        elif random_number in range(up[1]+1,up[2]):
            chromosome.clear()
            length = generate_random_number(4,8)
        ...
        #4 Splice of 2 randomly chosen best chromosomes
            ...
            new_chrom=[]
            for k in range(length):
                new_chrom.append(random.choice([int(x) for x in range(0,12) if x not in new_chrom]))
            chrom = list(set(new_chrom)-set(chromosome))
            for k in chrom:
                pos=generate_random_number(0,len(chromosome)-1)
                chromosome.insert(pos,k)
            path=4
        ...
        # Logging the Best and Worst Chromosomes
        if result>=13:
    ...
    print("Best chromosome",best_chromosome,"Best Fitness",best_fitness,"\n")
    print("Current chromosome",chromosome,"Fitness",result,"\n")
    fig = plt.figure(figsize=(50,30))
    plt.scatter(list([x for x in range(1,total_games+1)]), fitness,color='blue')
    #plt.plot(list([x for x in range(1,total_games+1)]), fitness, color='red')
    plt.title('Games Played vs Score')
